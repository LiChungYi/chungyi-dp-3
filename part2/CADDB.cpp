...
/**
* Create database / return point of database.
*@return instance(point of database).
*/
...
/** 
 * @param feature_string the string of some ShapeType
 *    
 * @return ShapeType of feature_string
 */ 
ShapeType get_shapetype_by_string(const std::string feature_string){
	...
	if( first_time == 1){
		...
		map_table["TRIANGLE"]   = TRIANGLE; // lwl 2010-05-11
	}
 ...
 * @param line the input string
 * @param &split_strings the vector of strings splited from the line
 * @return ShapeType of feature_string
 */ 
...
void CADDB::createLookUpTable(){
	...
	while( getline(inFeature, line)){		
		...
		if(!line.empty()){
			...
			const int shapetype_size[] = {4,4,4,4,3,7}; // lwl 2010-05-11
			...
			{
				for(int i=1;i<7;++i){
					attr.push_back(atof(split_strings.at(i).data())); // lwl 2010-05-11
				}
			}
			else 
			{
